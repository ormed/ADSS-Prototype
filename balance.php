...
<div id="wrapper">
    ...
    <div id="page-wrapper">
        ...
        <div class="container-fluid">
            ...
            <input type="button" id='graph_button' class="btn btn-info" value="Show Graph"
                   onClick='drawBasic(<?php echo count($results); ?>)'/>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped">
                            ...
                            foreach ($results as $index => $result) {
                                ...
                                <input type="hidden" id='hidden_balance_<?php echo $index ?>'
                                       value='<?php echo $balance[0]['TOTAL'] ?>'/>
                                <input type="hidden" id='hidden_date_<?php echo $index ?>'
                                       value='<?php echo($move_date[0]['MOVE_DATE']) ?>'/>
                                ...
                                <tr>
                                    ...
                                    <td><?php if (strcmp($result['ESSENCE'], "Credit") == 0) {
                                            echo("<font color='green'>" . (number_format(($result['QUANTITY'] * $price), 1, '.', ',')) . "$");
                                        } else {
                                            echo("<font color='red'>-" . (number_format(($result['QUANTITY'] * $price), 1, '.', ',')) . "$");
                                        } ?></font></td>
                                    <td><?php echo($result['ESSENCE']) ?></td>
                                    <?php if ($index == (count($results) - 1)) { ?>
                                        <td><h4><u><strong><?php if ($balance[0]['TOTAL'] > 0) {
                                                            echo "<font color='green'>";
                                                        } else {
                                                            echo "<font color='red'>";
                                                        }
                                                        echo(number_format($balance[0]['TOTAL'], 1, '.', ',') . "$") ?></font></strong></u>
                                            </h4></td>
                                    <?php } else { ?>
                                        <td><strong><?php if ($balance[0]['TOTAL'] > 0) {
                                                    echo "<font color='green'>";
                                                } else {
                                                    echo "<font color='red'>";
                                                }
                                                echo(number_format($balance[0]['TOTAL'], 1, '.', ',') . "$") ?></font></strong>
                                        </td>
                                    <?php } ?>
                                </tr>
                                <?php
