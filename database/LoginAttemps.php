...
class LoginAttemps {
    ...
    public static function get_failed_log_count($ip_address) {
        ...
        if (count($user) > 0) {
            ...
            $attempts++;
            $q = "UPDATE  `adss`.`login_attempts` SET  `attempts` =  '{$attempts}' WHERE  `login_attempts`.`id` ='{$user[0]['id']}';";
            $db->createQuery($q);

        } else {//not in DB so we add him
            $q = "INSERT INTO `adss`.`login_attempts` (`id`, `user_ip`, `attempts`, `lastlogin`)";
    ...
    public static function clear_attempts($ip_address) {
        $db = new Database();
        $q = "UPDATE  `adss`.`login_attempts` SET  `attempts` =  '{0}' WHERE  `login_attempts`.`user_ip` ='{$ip_address}';";
        $db->createQuery($q);
    }
